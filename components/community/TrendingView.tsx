...
const TrendingView: React.FC = React.memo(() => {
  ...
  const fetch = useCallback(async (pageNum: number, append: boolean) => {
    ...
    setError(null);
    try {
      const res = await communityApi.getTrending({ page: pageNum, limit: 20 });
  ...
  if (topics.length === 0) {
    return (
      <div className="text-center py-16 bg-gray-800/50 rounded-lg border border-gray-700">
        ...
        <p className="text-gray-400 text-lg mb-1">Nothing trending right now</p>
        <p className="text-gray-500 text-sm">Check back later for hot discussions.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-orange-400 text-sm font-semibold">Trending Now</span>
      </div>
