...
const SpotifyLinkDialog: React.FC = () => {
  const { user, isSpotifyPremium, refreshSpotifyProduct } = useAuth();
  ...
  useEffect(() => {
    const handleOpen = () => {
      if (!user || isSpotifyPremium) return;
      setSessionDismissed(false);
      setIsOpen(true);
    };

    window.addEventListener('spotify-link-dialog:open', handleOpen);
    return () => window.removeEventListener('spotify-link-dialog:open', handleOpen);
  ...
  }, [isConnecting, user, refreshSpotifyProduct]);
      ...
      <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-2xl border border-gray-600/50 max-w-md w-full animate-scale-in relative overflow-hidden">
        ...
        <div className="relative z-10 p-6 md:p-8">
          ...
          <p className="text-gray-300 text-center mb-6 leading-relaxed text-sm">
            Spotify Premium is required for full track playback. Connect your Spotify account to check your subscription and unlock the best experience.
          </p>
          ...
          <button
            ...
            className="w-full bg-green-600 hover:bg-green-500 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-green-900/30 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {isConnecting ? (
              <>
          ...
          <p className="text-gray-400 text-xs text-center mt-4">
            {isConnecting
              ? 'After connecting, we\'ll check your subscription status...'
